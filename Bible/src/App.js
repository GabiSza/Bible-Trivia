...
export const App = () => {
  ...
  const [userAnswer, setUserAnswer] = useState("1 John"); 
  ...
  const answer = async (e) => {
    e.preventDefault();
    const userA = await window.prompt("Type the book name");
    setUserAnswer(userA);
    setGuessCorrect((userA.trim().toLocaleLowerCase() === currentScripture.book_name.trim().toLocaleLowerCase()));
    setShowResult(true);
  }
  ...
  return (
    <div className='container text-center m-5'>
      ...
      <form className='form-group'>
        <div className='container'>
          <div className='d-flex'>
            ...
            <button  className='btn btn-info' onClick ={(e) => answer(e)}>Answer</button>
          </div>
      ...
      <div className='row text-center mt-5'>
        ...
        <h4 className='heading'> Your Answer: <span className="text-warning">{userAnswer}</span></h4>
